...
export function validateK6Summary(summary, expected) {
  ...
  for (const field of ["targetId", "probeId", "sourceSha"]) {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const [path, targetId, probeId, sourceSha] = process.argv.slice(2);
  if (!path || !targetId || !probeId || !sourceSha) throw new Error("usage: verify-k6-summary.mjs <path> <target> <probe> <sha>");
